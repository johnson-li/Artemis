...
case ${ROLE} in
    "balancer")
        sudo -b LD_LIBRARY_PATH="$HOME/app/bin" ~/app/bin/balancer --mysql=${DATABASE} --user='johnson' --password='welcOme0!' ${interface} 0.0.0.0 4433 ~/app/keys/server.key ~/app/keys/server.cert > $HOME/balancer.log 2>&1
