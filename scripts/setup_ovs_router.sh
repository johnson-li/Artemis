...
sudo ovs-vsctl add-port br1 gre -- set interface gre type=gre options:remote_ip=${server_ip0}
sudo ovs-ofctl add-flow br1 priority=200,udp,tp_dst=80,in_port=eth1,actions=gre


#sudo ip route add default via ${gateway} dev eth0 tab 1
#sudo ip route add default via ${gateway} dev br1 tab 2
#sudo ip rule add from ${ipv4_0}/24 tab 1 priority 500
