...
sudo ovs-vsctl add-port br1 gre_router -- set interface gre_router type=gre options:remote_ip=${router_ip0}
#sudo ovs-ofctl add-flow br1 priority=100,in_port=gre_router,actions=mod_nw_dst=${ipv4_br},mod_dl_dst=${mac_br},local
#sudo ovs-ofctl add-flow br1 priority=100,in_port=local,actions=gre_router
